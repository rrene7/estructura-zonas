...
$msg=isset($_GET['ok']) ? 'Unidad aprobada/asignada. Ya salio de la lista pendiente.' : ''; $err='';
...
try {
    if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['accion'] ?? '')==='asignar') {
        ...
        $pdo->commit();
        header('Location: asignar_zonas.php?zona_id='.(int)$zonaId.'&ok=1');
        exit;
    }
...
$excluirAsignadas = "AND BINARY ou.legacy_table <> BINARY 'MOI_CABECERA_ZONA' AND NOT EXISTS (SELECT 1 FROM organizational_units cabx WHERE cabx.id = ou.parent_id AND BINARY cabx.legacy_table = BINARY 'MOI_CABECERA_ZONA')";
$candidatos=$zona ? q($pdo, "SELECT ou.id,ou.code,ou.name,ou.parent_id,ou.legacy_table,ou.legacy_id,ut.name AS tipo,parent.name AS superior FROM organizational_units ou LEFT JOIN unit_types ut ON ut.id=ou.unit_type_id LEFT JOIN organizational_units parent ON parent.id=ou.parent_id WHERE ou.lifecycle_status='vigente' AND ou.id<>:cab $excluirAsignadas AND UPPER(ou.name COLLATE utf8mb4_unicode_ci) LIKE :pat $extra ORDER BY ou.name LIMIT 200", ['cab'=>$zona['cabecera_unit_id'] ?? 0,'pat'=>'%'.$clave.'%']) : [];
