...
$zonas = q($pdo, "SELECT z.id, z.zone_number, z.zone_label, z.normalized_name, cab.id AS cabecera_unit_id FROM moi_zonas_cabecera_vigentes z LEFT JOIN organizational_units cab ON cab.legacy_table='MOI_CABECERA_ZONA' AND cab.legacy_id=CAST(z.zone_number AS CHAR) WHERE z.lifecycle_status='vigente' ORDER BY z.zone_number");
$zonaId = (int)($_GET['zona_id'] ?? ($_POST['zona_id'] ?? ($zonas[0]['id'] ?? 0)));
$zona = one($pdo, "SELECT z.id, z.zone_number, z.zone_label, z.normalized_name, cab.id AS cabecera_unit_id FROM moi_zonas_cabecera_vigentes z LEFT JOIN organizational_units cab ON cab.legacy_table='MOI_CABECERA_ZONA' AND cab.legacy_id=CAST(z.zone_number AS CHAR) WHERE z.id=:id", ['id'=>$zonaId]);
...
try {
    if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['accion'] ?? '')==='asignar') {
        ...
        $target=one($pdo, "SELECT z.id, z.zone_label, cab.id AS cabecera_unit_id FROM moi_zonas_cabecera_vigentes z LEFT JOIN organizational_units cab ON cab.legacy_table='MOI_CABECERA_ZONA' AND cab.legacy_id=CAST(z.zone_number AS CHAR) WHERE z.id=:id", ['id'=>$targetId]);
        if (!$target || !$target['cabecera_unit_id']) { throw new RuntimeException('La zona seleccionada no tiene cabecera canonica.'); }
...
$extra='';
if (($zona['zone_number'] ?? 0)==8) { $extra=" AND UPPER(ou.name) NOT LIKE '%PANAMA OESTE%'"; }
$candidatos=$zona ? q($pdo, "SELECT ou.id,ou.code,ou.name,ou.parent_id,ou.legacy_table,ou.legacy_id,ut.name AS tipo,parent.name AS superior FROM organizational_units ou LEFT JOIN unit_types ut ON ut.id=ou.unit_type_id LEFT JOIN organizational_units parent ON parent.id=ou.parent_id WHERE ou.lifecycle_status='vigente' AND ou.id<>:cab AND UPPER(ou.name) LIKE :pat $extra ORDER BY CASE WHEN ou.parent_id=:cab2 THEN 0 ELSE 1 END, ou.name LIMIT 200", ['cab'=>$zona['cabecera_unit_id'] ?? 0,'cab2'=>$zona['cabecera_unit_id'] ?? 0,'pat'=>'%'.$clave.'%']) : [];
